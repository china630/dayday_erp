...
export default function NewCounterpartyPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setBusy(false);
    if (!res.ok) {
      setMsg(`${t("counterparties.createErr")}: ${await res.text()}`);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-2xl font-semibold text-gray-900 mt-4">{t("counterparties.newTitle")}</h1>
      </div>

      <form
        noValidate
        onSubmit={(e) => void onSubmit(e)}
        className="bg-white p-6 shadow-sm rounded-xl border border-slate-100 max-w-xl space-y-4"
      >
        <div>
